...
- **Character voices** are generated with Gemini TTS by
  `scripts/generate-voices.mts` from the lines in `src/app/media/voice-lines.ts`.
  Edit a line there, run `pnpm run gen:voices`, commit the `.m4a` outputs.
  Wrong-answer lines stay warm and point at the retry.
