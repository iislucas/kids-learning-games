...
- **Pictures** — the character sheets, and the map's ground tiles and scenery —
  are generated with an image model in the media studio (`/media`) and committed
  from there. `src/app/media/default-pack.ts` points at them. To change one,
  generate a new one in the studio and replace the file.
